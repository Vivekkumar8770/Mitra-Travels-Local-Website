...
const schema = z.object({ slug: z.string().trim().min(2).regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), title: z.string().trim().min(2), excerpt: z.string().trim().min(2), content: z.string().trim().min(2), category: z.string().trim().min(2), publishedAt: z.string().trim().min(8), active: z.boolean() });
