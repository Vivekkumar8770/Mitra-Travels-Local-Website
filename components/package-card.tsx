...
export function PackageCard({ item }: { item: TourPackage }) {
  return (
    <Link href={`/packages/${item.slug}`} className="package-card group" aria-label={`View ${item.title} itinerary`}>
      ...
      <div className="p-6">
        ...
        <div className="package-card-action mt-5 border-t border-slate-100 pt-4"><span className="package-itinerary-cta">View itinerary <ArrowUpRight className="size-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></span></div>
      </div>
    </Link>
  );
}
