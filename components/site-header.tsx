...
export function SiteHeader({ settings }: { settings: PublicSettings }) {
  ...
  return (
    <header className="site-header">
      ...
      <div className="header-actions hidden items-center gap-3 sm:flex"><a href={`https://wa.me/${settings.phoneRaw.replace("+", "")}`} className="btn btn-outline">WhatsApp</a><Link href="/contact#enquiry" className="btn btn-primary">Plan my trip</Link></div>
        <Sheet>
          ...
          <SheetContent className="border-l-white/10 bg-[#071a33] text-white">
            <SheetHeader><SheetTitle className="text-white">Explore Mitra Travels</SheetTitle></SheetHeader>
            <nav className="flex flex-col px-4" aria-label="Mobile navigation">{links.map(([label, href]) => <SheetClose key={href} asChild><Link href={href} className={`mobile-nav-link ${isActive(href) ? "is-active" : ""}`} aria-current={isActive(href) ? "page" : undefined}>{label}</Link></SheetClose>)}</nav>
            <div className="mt-auto space-y-3 p-4"><a href={`tel:${settings.phoneRaw}`} className="btn btn-outline w-full">Call {settings.phone}</a><SheetClose asChild><Link href="/contact#enquiry" className="btn btn-primary w-full">Plan my trip</Link></SheetClose></div>
          </SheetContent>
        </Sheet>
      </div>
    </header>
