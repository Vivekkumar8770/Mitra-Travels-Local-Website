...
export async function SiteFooter({ settings }: { settings: PublicSettings }) {
  const featuredPackages = (await getPublicPackages()).filter((item) => item.featured).slice(0, 4);
  let packages: Array<{ title: string; url: string }> = [];
  try { packages = JSON.parse(settings.footerPackagesJson || "[]") as Array<{ title: string; url: string }>; } catch { packages = []; }
  if (!packages.length) packages = featuredPackages.map((item) => ({ title: item.title, url: `/packages/${item.slug}` }));
  ...
  return (
    <footer className="site-footer bg-[#06162b] text-white">
      <div className="container-shell grid gap-10 py-14 md:grid-cols-[1.3fr_0.8fr_1fr_0.9fr]">
        ...
        <div><p className="footer-title">Explore</p><div className="mt-4 grid gap-3 text-sm text-white/70"><Link href="/packages">Tour packages</Link><Link href="/about">About us</Link><Link href="/blog">Travel blog</Link><Link href="/faqs">FAQs</Link><Link href="/contact">Contact Us</Link></div></div>
        <div><p className="footer-title">Popular packages</p><div className="footer-package-links mt-4 grid gap-3 text-sm text-white/70">{packages.map((item) => <Link key={`${item.title}-${item.url}`} href={item.url}>{item.title}</Link>)}{!packages.length && <Link href="/packages">Explore all tour packages</Link>}</div></div>
