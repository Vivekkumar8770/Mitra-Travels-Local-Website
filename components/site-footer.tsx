  ...
  return (
    <footer className="site-footer bg-[#06162b] text-white">
      <div className="container-shell grid gap-10 py-14 md:grid-cols-[1.3fr_0.8fr_1fr_0.9fr]">
        ...
        <div><p className="footer-title">Contact</p><div className="mt-4 grid gap-4 text-sm text-white/70"><a href={`tel:${settings.phoneRaw}`} className="flex gap-3"><Phone className="mt-0.5 size-4 shrink-0 text-orange-400" />{settings.phone}</a><a href={`mailto:${settings.email}`} className="flex gap-3"><Mail className="mt-0.5 size-4 shrink-0 text-orange-400" />{settings.email}</a><p className="flex gap-3"><MapPin className="mt-0.5 size-4 shrink-0 text-orange-400" />{settings.address}</p></div></div>
      </div>
